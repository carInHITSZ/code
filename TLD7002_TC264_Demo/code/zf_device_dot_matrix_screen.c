...
const uint32 dot_matrix_screen_row_pin[] = {DOT_MATRIX_SCREEN_SR0_PIN, DOT_MATRIX_SCREEN_SR1_PIN,
DOT_MATRIX_SCREEN_SR2_PIN, DOT_MATRIX_SCREEN_SR3_PIN, DOT_MATRIX_SCREEN_SR4_PIN, DOT_MATRIX_SCREEN_SR5_PIN,
DOT_MATRIX_SCREEN_SR6_PIN};
...
const uint8 tld7002_ascii_font_5x7[][7] = {{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},  //    0
        ...
        {0x00, 0x08, 0x04, 0x02, 0x04, 0x08, 0x00},  // < 28
        {0x00, 0x00, 0x3E, 0x00, 0x3E, 0x00, 0x00},  // = 29
        {0x00, 0x08, 0x10, 0x20, 0x10, 0x08, 0x00},  // > 30
        ...
        {0x00, 0x08, 0x14, 0x22, 0x00, 0x00, 0x00},  // ^ 62
...
void dot_matrix_screen_scan (void)
{
    ...
    if (0 == (entry_num % 2))
    {
        ...
        if (7 > display_row_now)
        {
            ...
            for (i = 0; 5 > i; i++)
            {
                if ((tld7002_ascii_font_5x7[dot_matrix_screen_data_backup[0] - ' '][display_row_now])
                        & ((uint16) 1 << (i + 1))) // (i + 1)中的+1 是为了去掉取模的第一列空白
                {
            ...
            for (; 10 > i; i++)
            {
                if ((tld7002_ascii_font_5x7[dot_matrix_screen_data_backup[1] - ' '][display_row_now])
                        & ((uint16) 1 << (i - 5 + 1))) // (i - 5 + 1)中的+1 是为了去掉取模的第一列空白
                {
                    tld7002_duty[i] = dot_matrix_screen_brightness;
                }
                else
            ...
            for (; 15 > i; i++)
            {
                if ((tld7002_ascii_font_5x7[dot_matrix_screen_data_backup[2] - ' '][display_row_now])
                        & ((uint16) 1 << (i - 10 + 1))) // (i - 10 + 1)中的+1 是为了去掉取模的第一列空白
                {
                    tld7002_duty[i] = dot_matrix_screen_brightness;
                }
                else
...
//用于显示元素；
void dot_matrix_screen_show_element (const char *str)
{
    uint8 i;
    for (i = 0; i < 3; i++)
    {
...
//用于显示方向；
void dot_matrix_screen_show_direction (const char str)
{
    dot_matrix_screen_data[2] = str;
}
//-------------------------------------------------------------------------------------------------------------------
//  函数简介      点阵屏亮度调节
