...
def result_callback(msg):
    global find_id_2,find_id_1,find_id,cal_symbol,zone_flag,shibie_flag,id_flag
    if(shibie_flag == 1):
        find_id_vlm = msg.data
    else:
        find_id_vlm = 0

    if(find_id_vlm<=8 and find_id_vlm >=1 and shibie_flag == 1):
        if(zone_flag==1):
            find_id_1 = find_id_vlm
            zone_flag = 2
            shibie_flag = 0
            find_id_vlm = 0
            print("vlm1 = ",find_id_1)
        elif(zone_flag==2):
            if(id_flag == 1):
                find_id_2 = find_id_vlm
                cal_id()
                print("vlm1 + vlm2 = ",find_id)
                zone_flag = 0
                shibie_flag = 0
                find_id_vlm = 0
                id_flag=0
...
class navigation_demo():
    ...
    def ar_track_cb(self,data):
        ...
        if(find_id_ar <= 8 and find_id_cb >= 1):
            if(zone_flag == 1):
                find_id_1 = find_id_ar
                zone_flag = 2
                shibie_flag = 0
                find_id_ar = 0
                print("fing_id_1_ar_track = ",find_id_1)
            elif(zone_flag == 2):
                if(id_flag == 1):
                    find_id_2 = find_id_ar
                    cal_id()
                    print("find_id_ar_track = ",find_id)
                    zone_flag = 0
                    shibie_flag=0
                    find_id_ar = 0
                    id_flag = 0
    ...
    def ar_cb(self, data):#没问题
        ...
        global find_id_1, find_id_2, zone_flag,find_id,shibie_flag,find_id_cb,id_flag
        point_msg = data
        if(shibie_flag == 1):
            find_id_cb = id_match(point_msg)
        
        #开始识别
        if(find_id_cb<=8 and find_id_cb >=1 ):
            if(zone_flag==1 ):
                find_id_1 = find_id_cb
                zone_flag = 2
                shibie_flag = 0
                find_id_cb = 0
                print("find_id_1_cb = ",find_id_1)
            elif(zone_flag == 2 ):
                if(id_flag == 1):
                    find_id_2 = find_id_cb
                    cal_id()
                    print("find_id_ar_cb = ",find_id)
                    zone_flag = 0
                    shibie_flag=0
                    find_id_cb = 0
                    id_flag = 0
...
def cal_id():#根据不同的运算模式来计算find_id，当作脚本文件执行
    global find_id_1,find_id_2,find_id,cal_symbol
    if(cal_symbol==1):
        find_id = find_id_1+find_id_2
    elif(cal_symbol==2):
        find_id = find_id_1-find_id_2
    elif(cal_symbol==3):
        find_id = find_id_1*find_id_2
...
if __name__ == "__main__":
    ...
    if (input == '1'):
        now_position=1#表示现在在第一个区域
        zone_flag = 1 #识别第一个区域的第一个识别点
        ...
        while(position_end == 0):
            ...
            if(now_position == 1):#第一个区域
                if(zone_flag == 1):#第一个区域第一个识别点
                    shibie_flag = 1
            if(now_position == 1):#第一个区域
                if(zone_flag ==2 and navi_flag == 1):#第一个区域第一个识别点由于前面写的，此时zone_flag = 2
                    navi.goto(goalshibie[1])#去第一个区域第二个识别点
                    shibie_flag = 1
                    id_flag = 1
                    navi_flag = 0
                    rospy.set_param('/top_view_shot_node/im_flag',1) #拍照识别算式，计算find_id_2
                    
            if(now_position == 2):#第二个区域
                if(zone_flag == 1):#第二个区域第一个识别点
                    navi.goto(goalshibie[2])#去第二个区域第一个识别点
                    shibie_flag = 1
                    rospy.set_param('/top_view_shot_node/im_flag',1) #拍照识别算式，计算find_id_2     
            if(now_position == 2):#第二个区域
                if(zone_flag == 2):#第二个区域第二个识别点
                    navi.goto(goalshibie[3])#去第二个区域第二个识别点
                    shibie_flag = 1
                    id_flag = 1
                    navi_flag = 0
                    rospy.set_param('/top_view_shot_node/im_flag',1) #拍照识别算式，计算find_id_2
                    
            if(now_position == 3):#第三个区域
                if(zone_flag == 1):#第三个区域第一个识别点
                    navi.goto(goalshibie[4])#去第三个区域第一个识别点
                    shibie_flag = 1
                    rospy.set_param('/top_view_shot_node/im_flag',1) #拍照识别算式，计算find_id_2
            if(now_position == 3):#第三个区域
                if(zone_flag == 2):#第三个区域第二个识别点
                    navi.goto(goalshibie[5])#去第三个区域第二个识别点
                    shibie_flag = 1
                    id_flag = 1
                    navi_flag = 0
                    rospy.set_param('/top_view_shot_node/im_flag',1) #拍照识别算式，计算find_id_2
            ...
            if(find_id==1):
                navi.goto(goals[0])#去第一个区域第一个任务点
                find_id = 0
                find_id_1 = 0
                find_id_2 = 0
                now_position = 2
                zone_flag = 1 #识别第二个区域的第一个识别点
            
            if(find_id==2):
                navi.goto(goals[1])#去第一个区域第二个任务点
                find_id = 0
                find_id_1 = 0
                find_id_2 = 0
                now_position = 2
            ...
            if(find_id==3):
                navi.goto(goals[2])#去第一个区域第三个任务点
                find_id = 0
                find_id_1 = 0
                find_id_2 = 0
                now_position = 2
            ...
            if(find_id==4):
                #navi.goto(goals[3])#去第二个区域第四个任务点
                find_id = 0
                find_id_1 = 0
                find_id_2 = 0
                now_position = 3
                zone_flag = 1 #识别第三个区域的第一个识别点
            
            if(find_id==5):
                #navi.goto(goals[4])#去第二个区域第五个任务点
                find_id = 0
                find_id_1 = 0
                find_id_2 = 0
                now_position = 3
            ...
            if(find_id==6):
                #navi.goto(goals[5])#去第二个区域第六个任务点
                find_id = 0
                find_id_1 = 0
                find_id_2 = 0
                now_position = 3
            ...
            if(find_id==7):
                #navi.goto(goals[6])#去第三个区域第七个任务点
                find_id = 0
                find_id_1 = 0
                find_id_2 = 0
                now_position = 4
                navi.goto(goalzhongzhuan[end_num])#终点11
            
            if(find_id==8):
                #navi.goto(goals[7])#去第三个区域第八个任务点
                find_id = 0
                find_id_1 = 0
                find_id_2 = 0
                now_position = 4
                navi.goto(goalzhongzhuan[end_num])#终点12
            
            if(find_id==9):
                #navi.goto(goals[8])#去第三个区域第九个任务点
                find_id = 0
                find_id_1 = 0
                find_id_2 = 0
                now_position = 4
                navi.goto(goalzhongzhuan[end_num])#终点13
