...
def SaveWave():
    ...
    for number, pinyin in target_alabo_numbers.items():
        if pinyin in input_pinyin:
            matched_number = number
            break
    ...
    print(f"匹配到的运算符: {matched_operator}，匹配到的数字: {matched_number}")
...
def main():
    global im_flag, result_pub, voice_pub, voice_str, voice_ok_pub
